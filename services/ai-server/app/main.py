...
from .frame_store import LatestFrameStore, StoredFrame
...
from .observability import InMemoryMetrics, structured_log
...
logger = logging.getLogger("smartfactory.ai_server")
store = InMemoryEventStore(maxlen=get_settings().event_store_maxlen)
...
def _rosbridge_subscription_hints(source: str, topic_exposure: dict[str, Any]) -> dict[str, Any]:
    """Return source-scoped browser subscription hints without touching rosbridge."""
...
def _topic_exposure_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    browser_topics = {
        topic
        for row in rows
        for topic in row["topic_exposure"]["allowed_browser_topics"]
    }
    ingest_topics = {
        topic
        for row in rows
        for topic in row["topic_exposure"]["allowed_ingest_topics"]
    }
    publish_topics = {
        topic
        for row in rows
        for topic in row["topic_exposure"]["allowed_publish_topics"]
    }
    control_topics = {
        topic
        for row in rows
        for topic in row["topic_exposure"]["control_topics_allowed"]
    }
...
def _overlay_publish_payload_preview_for_source(source: str) -> dict[str, Any]:
    ...
    frame = frame_store.latest(source)
...
def _ros_publish_readiness(source: str) -> dict[str, Any]:
    frame = frame_store.latest(source)
    overlay = overlay_cache.latest(source)
    ...
    return {
        ...
        "frame_age_s": round(_frame_age_s(frame), 3) if frame is not None else None,
        "overlay_ready": overlay_ready,
        "readiness_state": readiness_state,
        "overlay_visual_state": overlay_visual_state,
        "event_count": event_count,
        "publish_payload_preview": _overlay_publish_payload_preview_for_source(source),
...
def _ros_evidence_event_publish_readiness(source: str) -> dict[str, Any]:
    latest_events = store.latest(source=source, limit=1)
        ...
        rows.append(
            {
                ...
                "evidence_event_publish_readiness": _ros_evidence_event_publish_readiness(source),
                ...
                "publish_readiness": _ros_publish_readiness(source),
...
def _store_overlay_result(result: OverlayRenderResult) -> None:
    overlay_cache.add(result.metadata())
    with _overlay_images_lock:
        _overlay_images[result.source] = result


def _latest_overlay_image(source: str) -> OverlayRenderResult | None:
    with _overlay_images_lock:
        return _overlay_images.get(source)
...
def _detect_and_overlay_frame_snapshot(
    *,
    ...
    metrics.record_detect_image(event_count=len(events))
    return {
        "frame": frame,
        "events": events,
        "overlay": overlay,
...
def _detect_and_overlay_decoded_frame(
    *,
    ...
    source_health.record_frame(source, at=_now_dt())
    return _detect_and_overlay_frame_snapshot(
        frame=frame,
...
def _detect_model_events(
    *,
    ...
    if not _model_worker_enabled(settings):
        return []
    try:
        provider = _get_lift_roi_segmenter(
        ...
        # Fail closed: the marker/evidence path keeps working, but no model
        # candidates are emitted until the optional runtime is configured.
        return []
...
@app.get(
    "/api/v1/health",
    responses={500: ERROR_RESPONSE_OPENAPI},
)
...
def _vision_stream_source_entry(source: str, *, stream_metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "source": source,
        "has_frame": frame_store.latest(source) is not None,
        "has_overlay": overlay_cache.latest(source) is not None,
        **_frame_overlay_sync_status(source),
        ...
        "ros_publish_readiness": _ros_publish_readiness(source),
        "evidence_event_publish_readiness": _ros_evidence_event_publish_readiness(source),
        ...
        "stream_metrics": stream_metrics["by_source"].get(
            source,
            {
                "clients_total": 0,
                "active_clients": 0,
...
def vision_streams(source: str | None = Query(default=None, json_schema_extra=SOURCE_ID_OPENAPI_EXTRA)) -> dict[str, Any]:
    ...
    if source is not None:
        _ensure_known_source(source)
    stream_metrics = metrics.stream_snapshot()
    selected_sources = [
        source_id for source_id in settings.source_ids if source is None or source_id == source
    ]
    source_entries = [
        _vision_stream_source_entry(source_id, stream_metrics=stream_metrics)
    ...
    return {
        ...
        "summary": _vision_streams_summary(source_entries),
...
@app.get(
    ...
    responses={
        200: _json_response_openapi(
            "Lane B ROS2/domain-bridge handoff topic matrix",
            _ros_handoff_response_schema(),
        ),
    },
)
def vision_ros_topics(source: str | None = Query(default=None, json_schema_extra=SOURCE_ID_OPENAPI_EXTRA)) -> dict[str, Any]:
    ...
    if source is not None:
        _ensure_known_source(source)
    source_rows = [
        row for row in _source_topic_rows() if source is None or row["source"] == source
    ]
    return {
        "generated_at": _now_iso(),
        "requested_source": source,
        ...
        "evidence_event_publish_readiness_summary": _ros_evidence_event_publish_readiness_summary(source_rows),
        "notes": [
            "HTTP/MJPEG endpoints remain debug/fallback only.",
...
def _worker_tick_result_for_source(
    *,
    ...
    frame = frame_store.latest(source)
    if frame is None:
        metrics.record_worker_tick(source=source, status="no_frame")
        return {
            "source": source,
            "status": "no_frame",
            "frame_seq": None,
    ...
    if not force and frame_age_s > max_frame_age_s:
        metrics.record_worker_tick(source=source, status="stale_frame")
        return {
            "source": source,
            "status": "stale_frame",
            "frame_seq": frame.frame_seq,
    ...
    latest_overlay_metadata = overlay_cache.latest(source)
    overlay_frame_seq = (
    ...
    if not force and isinstance(overlay_frame_seq, int) and overlay_frame_seq >= frame.frame_seq:
        metrics.record_worker_tick(source=source, status="skipped")
        return {
            "source": source,
            "status": "skipped",
            "frame_seq": frame.frame_seq,
    ...
    metrics.record_worker_tick(source=source, status="processed")
    return {
        "source": source,
        "status": "processed",
        "frame_seq": frame.frame_seq,
...
@app.get(
    ...
    responses={
        200: _json_response_openapi(
            "Lane B read-only worker readiness snapshot",
            _worker_status_response_schema(),
        ),
...
def vision_worker_status(
    ...
    return {
        ...
        "max_frame_age_s": resolved_max_frame_age_s,
        "debug_only": True,
        "summary": _worker_status_summary(status_items),
...
@app.post(
    ...
    responses={400: ERROR_RESPONSE_OPENAPI, 422: ERROR_RESPONSE_OPENAPI, 500: ERROR_RESPONSE_OPENAPI},
)
...
def _debug_sources_summary(source_entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "with_overlay_count": sum(1 for item in source_entries if item["latest_overlay"] is not None),
        "overlay_lag_count": sum(
            1
            for item in source_entries
            if isinstance(item.get("overlay_lag_frames"), int)
...
def _debug_source_snapshot(source: str, *, stream_metrics: dict[str, Any], frame_stats: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "stream_metrics": stream_metrics["by_source"].get(source, _default_stream_metrics()),
        "drop_metrics": {
...
@app.get(
    ...
    responses={
        200: _json_response_openapi(
            "Lane B source/frame/overlay debug snapshot",
            _debug_sources_response_schema(),
        ),
        400: ERROR_RESPONSE_OPENAPI,
    },
...
def vision_debug_sources(source: str | None = Query(default=None, json_schema_extra=SOURCE_ID_OPENAPI_EXTRA)) -> dict[str, Any]:
    ...
    stream_metrics = metrics.stream_snapshot()
    frame_stats = frame_store.stats()
    source_entries = [
        _debug_source_snapshot(
            source=item,
    ...
    return {
        ...
        "primary_stream_plane": "rosbridge",
        "debug_only": True,
        "summary": _debug_sources_summary(source_entries),
        "topic_exposure_policy": _ros_topic_exposure_policy(),
        "topic_exposure_summary": _topic_exposure_summary(source_entries),
        "sources": source_entries,
    }
...
@app.post(
    ...
    responses={
        200: _json_response_openapi(
            "Latest-frame ingest debug response",
            _frame_ingest_response_schema(),
        ),
        400: ERROR_RESPONSE_OPENAPI,
        422: ERROR_RESPONSE_OPENAPI,
...
async def ingest_frame(
    ...
    try:
        frame = _store_latest_frame_from_bytes(
            source=source,
            payload=payload,
            content_type=content_type,
        )
    ...
    return {
        ...
        "overlay": None,
        "ingest_context": _frame_ingest_context(transport="http_debug", topic=None),
...
async def ingest_and_process_frame(
    ...
    if not force and isinstance(overlay_frame_seq, int) and overlay_frame_seq >= frame.frame_seq:
        return {
            ...
            "overlay": latest_overlay_metadata,
            "reason": "latest overlay already matches latest frame",
            "ingest_context": _frame_ingest_context(
...
@app.get(
    ...
    responses={
        200: _json_response_openapi(
            "Latest raw frame debug metadata",
            _latest_frame_response_schema(),
        ),
        400: ERROR_RESPONSE_OPENAPI,
...
def latest_overlay(source: str = Query(..., json_schema_extra=SOURCE_ID_OPENAPI_EXTRA)) -> dict[str, Any]:
    ...
    return {
        "generated_at": _now_iso(),
        "requested_source": source,
        "sync": _frame_overlay_sync_status(source),
        "overlay": overlay,
    }
...
async def _mjpeg_latest_overlay_generator(source: str, *, max_fps: int):
    last_frame_seq = 0
    last_send_at = 0.0
    min_interval_s = 1.0 / float(max_fps)
    metrics.record_stream_client_opened(source=source)
    try:
        while True:
            overlay = _latest_overlay_image(source)
            if overlay is None or overlay.frame_seq == last_frame_seq:
                metrics.record_stream_stale_poll(source=source)
            ...
            metrics.record_stream_frame_sent(source=source)
            yield (
                b"--frame\r\n"
        ...
        metrics.record_stream_client_closed(source=source)
...
def debug_overlay_mjpeg_stream(
    ...
    _ensure_known_source(source)
    if _latest_overlay_image(source) is None:
        raise HTTPException(status_code=404, detail=f"no overlay image available for source: {source}")
    return StreamingResponse(
        _mjpeg_latest_overlay_generator(source, max_fps=max_fps),
...
@app.get(
    "/api/v1/metrics",
    responses={200: METRICS_RESPONSE_OPENAPI, 400: ERROR_RESPONSE_OPENAPI},
)
def metrics_snapshot(source: str | None = Query(default=None, json_schema_extra=SOURCE_ID_OPENAPI_EXTRA)) -> dict[str, Any]:
    if source is not None:
        _ensure_known_source(source)
    raw_metrics = metrics.snapshot()
    return {
        "generated_at": _now_iso(),
        "requested_source": source,
        "metrics": _metrics_snapshot_for_source(raw_metrics, source),
        "event_store": store.stats(),
        "frame_store": _frame_store_stats_for_source(source),
    }
...
@app.post(
    ...
    responses={
        200: _json_response_openapi("Contract-valid LiftRoiEvidence v1", _lift_roi_openapi_schema()),
        400: ERROR_RESPONSE_OPENAPI,
        500: ERROR_RESPONSE_OPENAPI,
    },
)
def evaluate_lift_roi(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    metrics.record_lift_roi_evaluation(
...
async def evaluate_lift_roi_image(
    source: str = Form(..., json_schema_extra=SOURCE_ID_OPENAPI_EXTRA),
    operation: str = Form(...),
    roi_json: str = Form(...),
    image: UploadFile = File(...),
    ...
    stable_frames: int = Form(default=1),
    count_stable: bool = Form(default=False),
    lift_up: bool | None = Form(default=None),
    lift_down_complete: bool | None = Form(default=None),
    backoff_complete: bool | None = Form(default=None),
    dropped_item_count: int | None = Form(default=None),
    ...
    evidence_payload: dict[str, Any] = {
        ...
        "roi": _json_form_object(roi_json, "roi_json"),
...
@app.post(
    "/api/v1/detect/image",
    responses={
        200: _json_response_openapi("Image detection response with VisionEvent v1 events", _detect_image_response_schema()),
        400: ERROR_RESPONSE_OPENAPI,
        422: ERROR_RESPONSE_OPENAPI,
        500: ERROR_RESPONSE_OPENAPI,
    },
