...
from .openapi_schemas import ERROR_RESPONSE_OPENAPI, METRICS_RESPONSE_OPENAPI, SOURCE_ID_OPENAPI_EXTRA, _debug_sources_response_schema, _detect_image_response_schema, _frame_ingest_response_schema, _frame_process_response_schema, _json_response_openapi, _latest_frame_response_schema, _lift_roi_openapi_schema, _ros_handoff_response_schema, _synthetic_frame_response_schema, _vision_streams_response_schema, _worker_status_response_schema
...
_runtime_context_var: ContextVar[RuntimeContext] = ContextVar('smartfactory_ai_server_runtime_context', default=default_runtime_context)
...
def _detect_model_events(*, source: str, decoded_image, image_width: int, image_height: int) -> list[dict[str, Any]]:
    ...
    if not _model_worker_enabled(settings):
        return []
    try:
        provider = _get_lift_roi_segmenter(model_path=settings.vision_model_path, task=settings.vision_model_task, confidence=settings.vision_model_conf, iou=settings.vision_model_iou, image_size=settings.vision_model_imgsz, device=settings.vision_model_device, class_map_json=settings.vision_model_class_map_json, unmapped_class=settings.vision_model_unmapped_class)
        started = perf_counter()
...
def health() -> dict[str, Any]:
    settings = get_settings()
    source_summary = _runtime_context().source_health.summary(settings.source_ids, now=_now_dt(), stale_after_s=settings.source_stale_after_s, offline_after_s=settings.source_offline_after_s)
    lift_roi_model_status = _vision_model_status(settings)
    return {'service': 'ai-server', 'status': 'ok', 'service_version': SERVICE_VERSION, 'contract_version': settings.vision_event_schema_version, 'model_status': 'loaded', 'models': {'marker': {'status': 'loaded', 'name': settings.model_name}, 'lift_roi': {'status': lift_roi_model_status, 'task': settings.vision_model_task, 'path_configured': bool(settings.vision_model_path.strip()), 'device': settings.vision_model_device, 'worker_enabled': settings.vision_model_worker_enabled, 'worker_active': _model_worker_enabled(settings), 'class_map_configured': bool(settings.vision_model_class_map_json.strip()), 'unmapped_class': settings.vision_model_unmapped_class, 'max_events': settings.vision_model_max_events}}, 'source_summary': source_summary, 'event_retention': _runtime_context().store.stats(), 'version': SERVICE_VERSION, 'schema_version': settings.vision_event_schema_version, 'main_server_url': settings.main_server_url, 'sources': settings.source_ids}
...
def sources() -> dict[str, Any]:
    ...
    now = _now_dt()
    for source in settings.source_ids:
        health_snapshot = _runtime_context().source_health.snapshot(source, now=now, stale_after_s=settings.source_stale_after_s, offline_after_s=settings.source_offline_after_s)
        source_items.append({'source': source, 'source_id': source, 'kind': _source_kind(source), 'robot_id': _robot_id_for_source(source), 'enabled': health_snapshot.enabled, 'status': health_snapshot.status, 'frame_id': _frame_id_for_source(source), 'last_frame_at': health_snapshot.last_frame_at, 'last_frame_age_s': health_snapshot.last_frame_age_s, 'last_event_at': health_snapshot.last_event_at, 'last_event_kind': health_snapshot.last_event_kind, 'last_event_id': health_snapshot.last_event_id, 'last_marker_id': health_snapshot.last_marker_id, 'frame_count': health_snapshot.frame_count, 'event_count': health_snapshot.event_count, 'target_fps': _source_definition(source).target_fps or settings.source_target_fps, 'notes': _source_notes(source), 'ros_topic': _physical_input_topic_for_source(source), 'ros_message_type': _physical_input_message_type_for_source(source), 'ros_content_type': _physical_input_content_type_for_source(source)})
    return {'sources': source_items}

def latest_detections(source: str | None=Query(default=None, json_schema_extra=SOURCE_ID_OPENAPI_EXTRA), limit: int=Query(default=10, ge=1, le=50)) -> dict[str, Any]:
    settings = get_settings()
    if source is not None and source not in settings.source_ids:
        raise HTTPException(status_code=400, detail=f'unknown source: {source}')
    return {'generated_at': _now_iso(), 'events': _runtime_context().store.latest(source=source, limit=limit)}
...
def register_routes(app, *, runtime_context: RuntimeContext | None=None) -> None:
    ...
    app.get('/api/v1/detections/latest', responses={400: ERROR_RESPONSE_OPENAPI, 422: ERROR_RESPONSE_OPENAPI})(latest_detections)
    app.get('/api/v1/vision/streams', responses={200: _json_response_openapi('Main-facing HTTP/MJPEG Vision Stream Gateway discovery', _vision_streams_response_schema()), 400: ERROR_RESPONSE_OPENAPI})(vision_streams)
    app.get('/api/v1/vision/ros/topics', responses={200: _json_response_openapi('Lane B ROS2/domain-bridge handoff topic matrix', _ros_handoff_response_schema())})(vision_ros_topics)
    app.get('/api/v1/vision/worker/status', responses={200: _json_response_openapi('Lane B read-only worker readiness snapshot', _worker_status_response_schema()), 400: ERROR_RESPONSE_OPENAPI, 422: ERROR_RESPONSE_OPENAPI})(vision_worker_status)
    app.post('/api/v1/vision/worker/tick', responses={400: ERROR_RESPONSE_OPENAPI, 422: ERROR_RESPONSE_OPENAPI, 500: ERROR_RESPONSE_OPENAPI})(vision_worker_tick)
    app.get('/api/v1/vision/debug/sources', responses={200: _json_response_openapi('Lane B source/frame/overlay debug snapshot', _debug_sources_response_schema()), 400: ERROR_RESPONSE_OPENAPI})(vision_debug_sources)
    app.post('/api/v1/vision/frame', responses={200: _json_response_openapi('Latest-frame ingest debug response', _frame_ingest_response_schema()), 400: ERROR_RESPONSE_OPENAPI, 422: ERROR_RESPONSE_OPENAPI})(ingest_frame)
    app.post('/api/v1/vision/frame/process', responses={200: _json_response_openapi('Latest-frame ingest plus immediate overlay processing response', _frame_process_response_schema()), 400: ERROR_RESPONSE_OPENAPI, 422: ERROR_RESPONSE_OPENAPI})(ingest_and_process_frame)
    app.get('/api/v1/vision/frame/latest', responses={200: _json_response_openapi('Latest raw frame debug metadata', _latest_frame_response_schema()), 400: ERROR_RESPONSE_OPENAPI, 404: ERROR_RESPONSE_OPENAPI})(latest_frame)
    app.get('/api/v1/vision/frame/latest/image', responses={400: ERROR_RESPONSE_OPENAPI, 404: ERROR_RESPONSE_OPENAPI})(latest_frame_image)
    app.get('/api/v1/vision/overlay/latest', responses={400: ERROR_RESPONSE_OPENAPI, 404: ERROR_RESPONSE_OPENAPI})(latest_overlay)
    app.get('/api/v1/vision/overlay/latest/image', responses={400: ERROR_RESPONSE_OPENAPI, 404: ERROR_RESPONSE_OPENAPI})(latest_overlay_image)
    app.get('/api/v1/vision/stream/{source}.mjpeg', responses={400: ERROR_RESPONSE_OPENAPI, 404: ERROR_RESPONSE_OPENAPI})(debug_overlay_mjpeg_stream)
    app.get('/api/v1/metrics', responses={200: METRICS_RESPONSE_OPENAPI, 400: ERROR_RESPONSE_OPENAPI})(metrics_snapshot)
    app.post('/api/v1/vision/synthetic/frame', responses={200: _json_response_openapi('Synthetic frame detection response with overlay metadata', _synthetic_frame_response_schema()), 400: ERROR_RESPONSE_OPENAPI, 422: ERROR_RESPONSE_OPENAPI, 500: ERROR_RESPONSE_OPENAPI})(ingest_synthetic_frame)
    app.post('/api/v1/lift-roi/evaluate', responses={200: _json_response_openapi('Contract-valid LiftRoiEvidence v1', _lift_roi_openapi_schema()), 400: ERROR_RESPONSE_OPENAPI, 500: ERROR_RESPONSE_OPENAPI})(evaluate_lift_roi)
    app.post('/api/v1/lift-roi/evaluate-image', responses={200: _json_response_openapi('Contract-valid LiftRoiEvidence v1', _lift_roi_openapi_schema()), 400: ERROR_RESPONSE_OPENAPI, 503: ERROR_RESPONSE_OPENAPI, 500: ERROR_RESPONSE_OPENAPI})(evaluate_lift_roi_image)
    app.post('/api/v1/detect/image', responses={200: _json_response_openapi('Image detection response with VisionEvent v1 events', _detect_image_response_schema()), 400: ERROR_RESPONSE_OPENAPI, 422: ERROR_RESPONSE_OPENAPI, 500: ERROR_RESPONSE_OPENAPI})(detect_image)
