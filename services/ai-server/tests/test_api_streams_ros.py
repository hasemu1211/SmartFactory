...
def test_webrtc_compositor_metrics_path_resolves_relative_env_to_repo_root(monkeypatch):
    settings = get_settings()
    monkeypatch.setattr(settings, "vision_webrtc_compositor_metrics_dir", Path(".run/vision/compositor-metrics"))
    monkeypatch.setattr(settings, "vision_webrtc_sidecar_streams", "global_cam_01/full")
    monkeypatch.setattr(settings, "vision_webrtc_compositor_publisher_streams", "global_cam_01/full")
    monkeypatch.setattr(
        settings,
...
def test_vision_streams_declares_http_gateway_primary_and_internal_rosbridge_policy():
    ...
    assert webrtc["overlay_metadata"] == {
        ...
        "events_url": (
            "http://<vision-host>:8100/api/v1/detections/latest?"
            "source=global_cam_01&limit=20"
        ),
...
def test_webrtc_offer_falls_back_when_whep_returns_empty_or_invalid_sdp(
    monkeypatch, tmp_path
):
    from app.api import vision as vision_api
...
def test_stream_discovery_marks_webrtc_ready_when_sidecar_path_is_online(
    monkeypatch, tmp_path
):
...
def test_webrtc_demo_page_prefers_webrtc_and_contains_mjpeg_fallback_path():
    ...
    assert "/api/v1/vision/overlay/metadata?source=global_cam_01&view=full&limit=20" in response.text
